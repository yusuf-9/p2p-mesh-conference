...
DO $$ BEGIN
 CREATE TYPE "public"."media_handle_type" AS ENUM('p2p_mesh');
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint
...
CREATE TABLE IF NOT EXISTS "media_handles" (
	...
	"type" "media_handle_type" DEFAULT 'p2p_mesh' NOT NULL,
	"feed_type" "feed_type" DEFAULT 'camera' NOT NULL,
...
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "media_handles" ADD CONSTRAINT "media_handles_room_id_rooms_id_fk" FOREIGN KEY ("room_id") REFERENCES "public"."rooms"("id") ON DELETE cascade ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint
