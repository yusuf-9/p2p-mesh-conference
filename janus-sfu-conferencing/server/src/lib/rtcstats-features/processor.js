...
function extractSessionMetadata(dump, includedPCIds) {
...
function extractPeerConnectionMetadata(trace) {
...
export async function processRTCStatsDump(userId, filePath, processedDir) {
    ...
    for (const [pcId, trace] of Object.entries(dump.peerConnections)) {
        ...
        // Skip PCs that never produced stats (e.g. immediately-closed placeholder PCs)
        if (!trace.some(e => e.type === 'getStats')) continue;
        includedPCIds.push(pcId);
        pConnections[pcId] = extractPeerConnectionMetadata(trace);
    }
