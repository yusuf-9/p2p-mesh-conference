...
import type { SessionDescriptionInit, StatsIdMap, StatsObject } from './types.js';

/** Internal mutable stats blob used by compression routines. */
type StatsDelta = Record<string, any>;
 ...
 */
export function statsCompression(
  baseStatsInput: StatsObject | Map<string, Record<string, unknown>>,
  newStatsInput: StatsObject | Map<string, Record<string, unknown>>,
  statsIdMap: StatsIdMap,
): Record<string, unknown> {
    const baseStats = map2obj(baseStatsInput);
    const delta = JSON.parse(JSON.stringify(map2obj(newStatsInput))) as StatsDelta;
    ...
    Object.keys(delta).forEach((id) => {
        ...
        Object.keys(report).forEach((name) => {
            if (report[name] === baseStats[id][name]) {
                ...
                const nested = report[name] as Record<string, unknown>;
                const baseNested = baseStats[id]![name] as Record<string, unknown> | undefined;
                Object.keys(nested).forEach(key => {
                    if (baseNested && baseNested[key] === nested[key]) {
                        delete nested[key];
                    }
                });
 ...
 */
export function statsDecompression(
  baseStatsInput: unknown,
  delta: Record<string, unknown>,
): StatsObject {
    const baseStats = map2obj(baseStatsInput);
    const newStats = JSON.parse(JSON.stringify(map2obj(delta))) as StatsDelta;
    ...
    Object.keys(baseStats).forEach(id => {
        ...
        if (!newStats[id]) {
            ...
            const report = baseStats[id] as Record<string, unknown>;
            Object.keys(report).forEach(name => {
                if (!newStats[id]![name]) {
                    newStats[id]![name] = report[name];
                } else if (typeof report[name] === 'object' && report[name] !== null) {
                    const newObject = newStats[id]![name] as Record<string, unknown>;
                    newStats[id]![name] = report[name];
                    Object.keys(newObject).forEach(key => {
                        (newStats[id]![name] as Record<string, unknown>)[key] = newObject[key];
                    });
                }
            });
        }
    ...
    return newStats as StatsObject;
}
 ...
 */
function removeCertificateStats(stats: StatsDelta): void {
 ...
 * @param stats {stats} - JSON stats object.
 */
function removeObsoleteProperties(stats: StatsDelta): void {
    Object.keys(stats).forEach((id) => {
        const report = stats[id];
 ...
 */
export function descriptionCompression(
  baseDescription: SessionDescriptionInit | null | undefined,
  newDescription: SessionDescriptionInit,
 ...
 */
export function descriptionDecompression(
  baseDescription: SessionDescriptionInit | null | undefined,
  newDescription: SessionDescriptionInit,
): SessionDescriptionInit {
    if (!baseDescription || baseDescription.type !== newDescription.type ||
...
const methodTable: Record<string, string | number> = {
...
const methodTableDecompression: Record<number, string> = {
    1: 'getStats',
    // Legacy duplicate: id 65 also maps to getUserMediaOnFailure in upstream rtcstats.
    65: 'navigator.mediaDevices.getUserMediaOnFailure',
};
const reverseMethodTable = Object.keys(methodTable).reduce<Record<string | number, string>>((table, method) => {
...
export function decompressMethod(methodKey: string | number): string {
    return reverseMethodTable[methodKey] ?? String(methodKey);
}

// Table-based lookup for compression of stats types.
// Numbers must not be reassigned.
// https://w3c.github.io/webrtc-stats/#rtcstatstype-str*
const statsTypeTable: Record<string, number> = {
    'inbound-rtp': 1,
    'outbound-rtp': 2,
...
const reverseStatsTypeTable = Object.keys(statsTypeTable).reduce<Record<number, string>>((table, type) => {
...
export function decompressStatsType(typeKey: string | number): string {
    return reverseStatsTypeTable[Number(typeKey)] ?? String(typeKey);
}
...
const statsPropertyTable: Record<string, string | number> = {
...
// Properties that are decompressed but for release purpose not yet compressed.
// These should be moved to the compression table at major version bumps.
const statsPropertyTableDecompression: Record<number, string> = {
...
const reverseStatsPropertyTable = Object.keys(statsPropertyTable).reduce<Record<string | number, string>>((table, property) => {
...
export function decompressStatsProperty(property: string | number): string {
    return reverseStatsPropertyTable[property] ?? String(property);
}

export const computePressureTable: Record<string, number> = {
    nominal: 4,
    fair: 3,
    serious: 2,
    critical: 1,
